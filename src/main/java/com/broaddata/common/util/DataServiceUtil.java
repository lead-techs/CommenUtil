...
{      
    ...
    public static void storeVideoDataobject(int targetDataobjectType,String dataobjectId,int nodeOSType,ByteBuffer contentStream,DataService.Client dataService,CommonService.Client commonService,int organizationId,int sourceApplicationId,int datasourceType,int datasourceId,String filename,String originalSourcePath,int targetRepositoryId,boolean needToExtractContentTexts,Map<String,String> metadataData) throws Exception
    {
        ...
        try
        {
            ...
            //boolean isContentExisting = dataService.isContentExisting(organizationId,obj.getContentIds().get(0));
            ...
            {
                ContentStoreUtil.storeContent(organizationId,dataService,nodeOSType,contentIds.get(0),contentStream,objectStorageServiceInstance,objectStoreServiceProviderProperties);
                contentCopied = true;
            }
            else
            {
            ...
            dataService.storeDataobjects(organizationId,TargetRepositoryType.DATA_REPOSITORY.getValue(),targetRepositoryId,list,0,true);
        }
        catch (Exception e)
        {
