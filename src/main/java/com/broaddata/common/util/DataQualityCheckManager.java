...
public class DataQualityCheckManager 
{      
    ...
    private int incrementColumnType;
    private String incrementColumnLastProcessedItem;
    private String lastProcessedItemInDB;
    
    private boolean needPagenation;
    private Map<String,Long> duplicatedDataMap = null;
    
    public DataQualityCheckManager(DataServiceConnector dsConn,DataService.Client dataService,DataQualityCheckJob job,DataQualityCheckTask task) 
    {
        this.dsConn = dsConn;
    ...
    {           
        ...
        try
        {
            ...
            incrementColumn = dataQualityCheckConfig.getIncrementCheckColumn();
            incrementColumnType = dataQualityCheckConfig.getIncrementCheckColumnType();
            incrementColumnLastProcessedItem = task.getLastRunInfo();
            
            if ( incrementColumnLastProcessedItem == null || incrementColumnLastProcessedItem.trim().isEmpty() )
                incrementColumnLastProcessedItem = "";
            ...
            {                  
                ...
                {                            
                    ...
                    do
                    {
                        ...
                        if ( !incrementColumn.isEmpty() )
                        {
                            if ( incrementColumnType == MetadataDataType.TIMESTAMP.getValue() )
                                lastProcessedItemInDB = Tool.convertDateToTimestampString(resultSet.getTimestamp(incrementColumn));
                            else
                                lastProcessedItemInDB = resultSet.getString(incrementColumn);
                        }
                    }
                    ...
                    try
                    {
                        ...
                        if ( !incrementColumn.isEmpty() )        
                        {
                            String sql = String.format("select * from %s where CHECK_TASK_ID=%d and CHECK_JOB_ID=%d and DEFINITION_ID=%d and DATA_NAME='%s'",summaryTableName,task.getId(),job.getId(),definition.getId(),dataQualityCheckConfig.getTargetDatabaseItem());
                            log.info(" summary table sql="+sql);

                            PreparedStatement stmt1 = resultDbConn.prepareStatement(sql);
                            ResultSet result = JDBCUtil.executeQuery(stmt1);
                            ...
                            {
                                   // get data from sql
                                String oldColumnDataStr = result.getString("COLUMN_DATA");
                                String[] vals = oldColumnDataStr.split("\\;");
                                
                                long[][] oldColumnData = new long[columnSize][10];
                                
                                for(int ii=0;ii<vals.length;ii++)
                                {
                                    String[] val1s = vals[ii].split("\\,");
                                    
                                    for(int jj=0;jj<val1s.length-1;jj++)
                                    {
                                        oldColumnData[ii][jj] = Long.parseLong(val1s[jj+1]);
                                    }
                                }
                                ...
                                for(int ii=0;ii<columnSize;ii++)
                                {
                                    for(int jj=0;jj<10;jj++)
                                    {
                                        columnData[ii][jj] += oldColumnData[ii][jj];
                                    }
                                }
                                ...
                                long oldTotalRows = result.getLong("total_rows");
                                long oldCorrectRows = result.getLong("correct_rows");
                                long oldDuplicatedRows = result.getLong("duplicated_rows");
                                long oldHasProblemRows= result.getLong("has_problem_rows");
                                
                                String updateSummarySql = String.format("UPDATE %s set total_rows=?,correct_rows=?,duplicated_rows=?,has_problem_rows=?,column_data=?,check_time=? "
                                        + "where check_task_id=%d and check_job_id=%d and definition_id=%d and data_name='%s'",summaryTableName,task.getId(),job.getId(),definition.getId(),dataQualityCheckConfig.getTargetDatabaseItem());

                                resultStatement = resultDbConn.prepareStatement(updateSummarySql);
                                resultStatement.setObject(1, totalRows+oldTotalRows,MetadataDataType.LONG.getSqlType());
                                resultStatement.setObject(2, correctRows+oldCorrectRows,MetadataDataType.LONG.getSqlType());
                                resultStatement.setObject(3, duplicatedRows+oldDuplicatedRows,MetadataDataType.LONG.getSqlType());
                                resultStatement.setObject(4, hasProblemRows+oldHasProblemRows,MetadataDataType.LONG.getSqlType());
                                resultStatement.setObject(5, columnDatas, MetadataDataType.STRING.getSqlType());
                                resultStatement.setObject(6, checkTime, MetadataDataType.TIMESTAMP.getSqlType());
                    ...
                    while(true)
                    {
                        ...
                        try
                        {
                            if ( incrementColumn.isEmpty() )
                                ...
                                dataService.updateClientJob(ClientJobType.DATA_QUALITY_CHECK_JOB.getValue(), job.getOrganizationId(), job.getId(), 0, "",JobItemProcessingStatisticType.PROCESSED_ITEMS.getValue(), processed,0,lastProcessedItemInDB);
                            
                            break;
                        }
                        catch(Exception e)
                        {
                            log.error(" update client job failed! e="+e+" retry="+retry);
    ...
    private String generateQuerySQL(DatabaseType databaseType,long startLine,int pageSize,String tableName) // 1,1000; 1001,1000
    {
        ...
        {
            if ( incrementColumnLastProcessedItem.isEmpty() )
            {
                if ( incrementColumnType == MetadataDataType.TIMESTAMP.getValue() )
                    incrementColumnLastProcessedItem = "1970-01-01 00:00:00";
                else    
                    incrementColumnLastProcessedItem = "0";
            }

            if ( filter==null || filter.trim().isEmpty() )
            {
                if ( incrementColumnType == MetadataDataType.STRING.getValue() )
                    querySql = String.format("select * from %s%s where %s>'%s' order by %s",schema,tableName, incrementColumn, incrementColumnLastProcessedItem, incrementColumn );
                else
                if (  incrementColumnType == MetadataDataType.TIMESTAMP.getValue() )
                {
                    if ( (databaseType == DatabaseType.ORACLE || databaseType == DatabaseType.ORACLE_SERVICE) )
                        querySql = String.format("select * from %s%s where %s>%s order by %s",schema,tableName, incrementColumn, Tool.getOralceDatetimeFormat(incrementColumnLastProcessedItem,"yyyy-MM-dd hh24:mi:ss"), incrementColumn );
                    else
                        querySql = String.format("select * from %s%s where %s>'%s' order by %s",schema,tableName,incrementColumn, incrementColumnLastProcessedItem,incrementColumn );
                }            
                else
            ...
            {   
                if ( incrementColumnType == MetadataDataType.STRING.getValue() )
                    querySql = String.format("select * from %s%s where %s>'%s' and ( %s ) order by %s",schema,tableName, incrementColumn, incrementColumnLastProcessedItem, filter, incrementColumn);
                else
                if (  incrementColumnType == MetadataDataType.TIMESTAMP.getValue() )
                {
                    if ( (databaseType == DatabaseType.ORACLE || databaseType == DatabaseType.ORACLE_SERVICE) )
                        querySql = String.format("select * from %s%s where %s>%s and ( %s ) order by %s",schema,tableName, incrementColumn, Tool.getOralceDatetimeFormat(incrementColumnLastProcessedItem,"yyyy-MM-dd hh24:mi:ss"), filter, incrementColumn );
                    else
                        querySql = String.format("select * from %s%s where %s>'%s' and ( %s ) order by %s",schema,tableName,incrementColumn, incrementColumnLastProcessedItem,filter, incrementColumn );
                }            
                else
    ...
    private String generateCountQuerySQL() // 1,1000; 1001,1000
    {
        ...
        else
        {
            ...
            if ( incrementColumnType == MetadataDataType.STRING.getValue() )
                querySql = String.format("select count(*) from %s%s where %s>'%s'",schema,databaseItem, incrementColumn, incrementColumnLastProcessedItem);
            else
            if (  incrementColumnType == MetadataDataType.TIMESTAMP.getValue() )
            {
                if ( (databaseType == DatabaseType.ORACLE || databaseType == DatabaseType.ORACLE_SERVICE) )
                    querySql = String.format("select count(*) from %s%s where %s>%s",schema,databaseItem, incrementColumn, Tool.getOralceDatetimeFormat(incrementColumnLastProcessedItem,"yyyy-MM-dd hh24:mi:ss"));
                else
                    querySql = String.format("select count(*) from %s%s where %s>'%s'",schema,databaseItem,incrementColumn, incrementColumnLastProcessedItem);
            }            
            else
