...
public class ObjectRecognitionProcessor extends ModelProcessor
{
    static final Logger log = Logger.getLogger("ObjectRecognitionProcessor");
    ...
    public Map<String,Object> execute() throws Exception
    {
      ...
      /*  map = Util.getEDFVideoProcessingService(platformEm,model.getServiceInstanceId());
        ...
        for(String row : rows)
        {
            ...
            String objectItemName = columns[2];
         
            int objectTypeId = VideoProcessUtil.getObjectTypeIdByItem(videoObjectFeatureValueList,objectItemName,model.getObjectDetectionModelId()); 
            
            if ( objectTypeId == 1 || objectTypeId == 2 )
            {
                log.info(" skip person and viehcle");
                continue;
            }
            ...
            //String objectColor = detectObjectColor(mlManager,filename,position);
            
            String valueInDBFormat = VideoProcessUtil.convertToDBFormat(platformEm,model.getObjectDetectionModelId(),"item",objectItemName);
            features.put("1", valueInDBFormat);

            //valueInDBFormat = VideoProcessUtil.convertToDBFormat(platformEm,model.getObjectDetectionModelId(),"object_color",objectColor);
            //features.put("2", "");
    ...
    String detectObjectColor(MalongManager mlManager,String pictureFilepath, Map<String,Integer> position)
    {
        String carColor = "";
